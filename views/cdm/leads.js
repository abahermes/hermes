$(function() {
  $("#frmCtcDtls,#frmSaveUpdDelLead,#frmCtcDtls,#frmSelectBusinessType").draggable();
  $("#txtSearch").focus();
  ...
  $("#btnDelete").on("click", function(){
    ...
    if(leadid == ""){
      alert("Lead is not anymore existed! Please refresh your leads module to reflect if lead is still existed.");
  ...
  $("#btnBulkDelete").on("click", function(){
    $("#contacts,#dupLeads,#btnBulkDelete,#btnLeads").hide();
    $("#bulkCtcs,#btnCanDelete,#btnDelLeads,#btnDuplicateLeads").show();
  });

  $("#btnCanDelete").on("click", function(){
    $("#contacts,#btnBulkDelete").show();
    $("#bulkCtcs,#btnCanDelete,#btnDelLeads,#btnLeads").hide();
  });

  $("#btnDuplicateLeads").on("click", function(){
    $("#dupLeads,#btnBulkDelete,#btnLeads").show();
    $("#contacts,#bulkCtcs,#btnCanDelete,#btnDelLeads,#btnDuplicateLeads").hide();
  })

  $("#btnLeads").on("click", function(){
    $("#dupLeads,#btnLeads,#btnCanDelete,#btnDelLeads").hide();
    $("#contacts,#btnDuplicateLeads,#btnBulkDelete").show();
  });

  $("#btnSaveDupLead").on("click", function(){
    var url = getAPIURL() + 'leads.php';
    var f = "saveDuplicateLead";
    var userid = $("#userid").val();
    var leadid = $("#dupleadid").val();

    var data = { "f":f, "leadid":leadid, "userid":userid };
      ...
      onBlock: function() {
        $.ajax({
          ...
          ,success: function(data){
            // console.log(data);
            // return false;
            $("#frmSaveUpdDelLead").modal('hide');
            var leads = data['leads']['rows'];
            var leadscnt = leads.length;
            var dupleads = data['dupleads']['rows'];
            var dupcnt = dupleads.length;
            renderLeadsTbl(leads);
            renderDuplicateLeadsTbl(dupleads);
            var dupcnt = dupleads.length;
            $("#btnLeads").val("Leads List("+leadscnt+")");
            $("#btnDuplicateLeads").val("Duplicate Leads("+dupcnt+")");
            $("#rownum").html(addCommas(leadscnt));
            if(dupcnt > 0){
              $("#btnDuplicateLeads").val("Duplicate Leads ("+dupcnt+")");
              $("#btnDuplicateLeads").show();
            }
          }
  ...
  $("#btnExistIndAcct").on("click", function(){
    window.location = 'cdm.php?id='+ $("#acctid_i").val();
  });

  $("#btnExistCorpAcct").on("click", function(){
    window.location = 'cdm.php?id='+ $("#acctid_c").val();
  });

  $("#btnNewIndAcct,#btnNewCorpAcct").on("click", function(){
    $("#frmSelectBusinessType").modal('hide');
    $("#frmCtcDtls").modal('show');
  });

  $("#btnCanSelectBusType").on("click", function(){
    $("#frmSelectBusinessType").modal('hide');
  });

  $("#btnDelDupLead").on("click", function(){
    var url = getAPIURL() + 'leads.php';
    var f = "deleteDuplicateLead";
    var userid = $("#userid").val();
    var leadid = $("#dupleadid").val();
  ...
  $("#btnCanDupLead").on("click", function(){
    $("#dupleadid").val("");
    $("#frmSaveUpdDelLead").modal('hide');
  });

  $("#btnDelLeads").on("click", function(){
    var url = getAPIURL() + 'leads.php';
    var f = "deleteLeads";
    var userid = $("#userid").val();
    var ids = "";
    ...
    // var leadids = "";
    $("input:checkbox[name=chkLeadid]:checked").each(function() {
         selected.push($(this).val());
         // leadids += $(this).val() + ',';
    });

    if(selected.length == 0){
      alert("Please select lead(s) to delete.");
      return false;
    }

    // ids = leadids.substring(0,leadids.length - 1)
    // console.log(ids);

    var data = { "f":f, "userid":userid, "leadids":selected };
    // console.log(data);
    // return false;
      ...
      onBlock: function() {
        $.ajax({
          ...
          ,success: function(data){
            ...
            $("#contacts,#btnBulkDelete").show();
            $("#bulkCtcs,#btnCanDelete,#btnDelLeads").hide();
          }
          ,error: function(request, status, err){
            // console.log(request);
            // console.log(status);
  ...
  $("#txtCompany").change(function(){
    ...
    $("#rBusinessTypec").attr("disabled",false);
    if( $("#txtCompany").val() == "" ){
      $("#rBusinessTypec").attr("checked",false);
  ...
  $.ajax({
    ...
    ,success: function(data){
      ...
      if(leadscnt > 0){
        ...
        renderDuplicateLeadsTbl(dupleads);
        $("#btnBulkDelete").show();
        // $("#rownum").html('<b>'+ data['cnt'] +'</b> Leads contact found');
        // $.unblockUI();
      }else{
        $.unblockUI();
        $("#frmpopupmsg").modal({
...
function renderLeadsTbl(data){
  ...
  // console.log(data);
  // return false;
  var fullname = "return sortLeads('fullname')";
  var jobtl =  "return sortLeads('jobtitle')";
  var companyname ="return sortLeads('companyname')";
  var emailadd = "return sortLeads('emailadd')";

  // rownum += '<p>Showing <b>'+ data.length +'</b> Leads of </p>';
  html += '<table class="table table-striped jambo_table table-condensed table-hover table-bordered">';
    html += '<thead>';
      html += '<tr>';
        html += '<th width="2%">#</th>';
        html += '<th width="15%" style="cursor: pointer;" onClick="' + fullname + '">Name</th>';
        html += '<th width="15%" style="cursor: pointer;" onClick="' + jobtl + '">Job Title</th>';
        html += '<th width="18%" style="cursor: pointer;" onClick="' + companyname + '">Company</th>';
        html += '<th width="15%" style="cursor: pointer;" onClick="' + emailadd + '">e-Address</th>';
        html += '<th width="10%">Phone No</th>';
        html += '<th width="20%">Address</th>';
        html += '<th width="5%">ics</th>';
      html += '</tr>';
    html += '</thead>';
    html += '<tbody>';
    
    html1 += '<table class="table table-striped jambo_table table-condensed table-hover table-bordered">';
    html1 += '<thead>';
      html1 += '<tr>';
        html1 += '<th  width="2%"><input type="checkbox" id="chkAll" name="chkAll" /></th>';
        html1 += '<th width="15%">Name</th>';
        html1 += '<th width="15%">Job Title</th>';
        html1 += '<th width="18%">Company</th>';
        html1 += '<th width="15%">e-Address</th>';
        html1 += '<th width="10%">Phone No</th>';
        html1 += '<th width="20%">Address</th>';
        html1 += '<th width="5%">ica</th>';
      html1 += '</tr>';
    html1 += '</thead>';
    html1 += '<tbody>';
    ...
    for(var i=0; i<data.length;i++){
      ...
      comp = data[i]['companyname'] == "" || data[i]['companyname'] == null ? "" : data[i]['companyname'];
      addr = data[i]['address'] == "" || data[i]['address'] == null ? "" : data[i]['address'];
      phno = data[i]['mobilephoneno'] == "" || data[i]['mobilephoneno'] == null ? "" : data[i]['mobilephoneno'];
      email = data[i]['emailaddress'] == "" || data[i]['emailaddress'] == null ? "" : data[i]['emailaddress'];
      leadid = "'"+ data[i]['leadid'] + "'";
      fn = data[i]['firstname'] == "" || data[i]['firstname'] == null ? "" : data[i]['firstname'] +' ';
      mn = data[i]['middlename'] == "" || data[i]['middlename'] == null ? "" : data[i]['middlename'];
      ln = data[i]['lastname'] == "" || data[i]['lastname'] == null ? "" : data[i]['lastname'] +' ';
      name = ln + fn + mn;
      ...
      html1 += '<tr>';
        html1 += '<td class="text-center"><input type="checkbox" id="chkLeadid" name="chkLeadid" value="'+ data[i]['leadid'] +'"></td>';
        html1 += '<td>'+ name +'</td>';
        html1 += '<td>'+ jobtitle +'</td>';
        html1 += '<td>'+ comp +'</td>';
        html1 += '<td>'+ email +'</td>';
        html1 += '<td>'+ phno +'</td>';
        html1 += '<td>'+ address +'</td>';
        html1 += '<td class="text-center"></td>';
      html1 += '</tr>';
      cnt++;
    }
  ...
  $("#contacts").html(html);
  $("#bulkCtcs").html(html1);
  $("#rowres").html(addCommas(data.length));
...
function renderDuplicateLeadsTbl(data){
  var html = "";
  var cnt = 1;
  var rownum = "";
  var address = "";
  ...
  html += '<table class="table table-striped jambo_table table-condensed table-hover table-bordered">';
    html += '<thead>';
      html += '<tr>';
        ...
        html += '<th width="15%">Job Title</th>';
        html += '<th width="20%">Company</th>';
        html += '<th width="15%">e-Address</th>';
        html += '<th width="10%">Phone No</th>';
        html += '<th width="20%">Address</th>';
      html += '</tr>';
    html += '</thead>';
    ...
    for(var i=0; i<data.length;i++){
      ...
      html += '<tr onClick="return actLead('+ data[i]['id'] +');" data-toggle="modal" data-target="#frmSaveUpdDelLead" style="cursor: pointer;">';
        html += '<td class="text-center">'+ cnt +'</td>';
        html += '<td>'+ name +'</td>';
        html += '<td>'+ jobtitle +'</td>';
        html += '<td>'+ comp +'</td>';
    ...
    onBlock: function() {
      $.ajax({
        ...
        ,success: function(data){
          ...
          var accts = data['accts']['rows'];
          var leadname = lead['lastname']+' '+lead['firstname'];
          $("#bustypeacctname").html(leadname);
          $("#acctid_i").val("");
          $("#acctid_c").val("");
          $("#btnNewIndAcct,#btnExistIndAcct,#btnNewCorpAcct,#btnExistCorpAcct").hide();
          
          switch(accts.length){
            case 1:
                createPipeline(lead);
                $("#frmCtcDtls").modal('hide');
                switch(accts[0]['businesstype']){
                  case "c": 
                      $("#acctid_c").val(accts[0]['sesid']); 
                      $("#btnNewIndAcct,#btnExistCorpAcct").show();
                      $("#rBusinessTypei").attr("checked",true);
                      $("#rBusinessTypec").attr("disabled",true); 
                    break;
                  default: 
                      $("#acctid_i").val(accts[0]['sesid']); 
                      $("#btnExistIndAcct,#btnNewCorpAcct").show(); 
                      $("#rBusinessTypec").attr("checked",true);
                      $("#rBusinessTypei").attr("disabled",true); 
                    break;
                }
                $("#frmSelectBusinessType").modal('show');
                $.unblockUI();
              break;
            case 2:
                $("#frmCtcDtls").modal('hide');
                for(var i=0;i<accts.length;i++){
                  switch(accts[i]['businesstype']){
                    case "c": $("#acctid_c").val(accts[i]['sesid']); break;
                    default: $("#acctid_i").val(accts[i]['sesid']); break;
                  }
                }
                $("#btnExistIndAcct,#btnExistCorpAcct").show();
                $("#frmSelectBusinessType").modal('show');
                $.unblockUI();
              break;
            default: 
...
function saveProst(){
  ...
  var website = $("#txtWebsite").val() == "" || $("#txtWebsite").val() == null ? "" : $("#txtWebsite").val();
  ...
  var data = { "f": f, "abauser":abauser, "assignedto":assignedto, "userid":userid, "ini":ini, "leadid":leadid,
              ...
              "companyname":comp, "jobtitle":jt, "eaddr":eaddr, "website":website, "homphno":homphno, "mobphno":mobphno, "addr":addr, "nationality":nat, "ethnicity":eth, 
  ...
  $.blockUI({ 
    ...
    onBlock: function() {
      $.ajax({
        ...
        ,success: function(data){
          // console.log(data);
          var searchedleads = data['searchedleads']['rows'];
          renderLeadsTbl(searchedleads);
          $.unblockUI();
          // return false;
        }
        ,error: function(request, status, err){
