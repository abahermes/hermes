  ...
  function renderContacts() {
    ...
    getUserContacts(function(contacts, error){
      if (error) {
        ...
        // return false;
        var cntctcs = contacts.length;
        if(cntctcs >= 1000){
          alert("The system detected that you have existing contacts saved in your microsoft outlook with more than 1000 records. Please use the manual import of your contacts.");
          window.location = "leads.php";
          return false;
        }

        // if(confirm('Hermes will now get your Microsoft outlook contacts. Please click OK to proceed.') == true){
        // }else{
        //   window.location = "leads.php";
        //   return false;
        // }
        ...
        var row = [];
        var eaddr = new Array();
        // "emailaddress":contacts[i]['emailAddresses'][0],
        // "emailaddress":eaddr,
        for(var i=0;i<cntctcs;i++){
          // eaddr = "";
          // if(contacts[i]['emailAddresses'].length > 0){
          //   for(var a=0;a<contacts[i]['emailAddresses'].length;a++){
              // eaddr = contacts[i]['emailAddresses.'+0+'.address'];
          //   }
          // }
          // if( (contacts[i]['givenName'] == "" || contacts[i]['givenName'] == null)
          //       && (contacts[i]['surname'] == "" || contacts[i]['surname'] == null)
          //       && (contacts[i]['companyName'] == "" || contacts[i]['companyName'] == null) ){
          // }else{
            row = { "firstname":contacts[i]['givenName'], "lastname":contacts[i]['surname'], "middlename":contacts[i]['middleName'],
                  "chinesename":contacts[i]['generation'], "initials":contacts[i]['initials'], "birthdate":contacts[i]['birthday'],
                  "companyname":contacts[i]['companyName'], "jobtitle":contacts[i]['jobTitle'], "manager":contacts[i]['manager'],
                  "mobilephone":contacts[i]['mobilePhone'], "eaddr":contacts[i]['emailAddresses']
                };
            rows.push(row);
          // }
        }
        var data = { "f":f, "ctcs":rows, "userid":userid };
        console.log(data);
        // return false;
        ...
        $.ajax({
          ...
          ,success: function(data){
            console.log(data);
            return false;
            alert("Your leads successfully sync to hermes.");
            // return false;
            window.location = "leads.php";
            // return false;
          }
    ...
    getAccessToken(function(accessToken) {
      if (accessToken) {
        ...
        client
          ...
          .api('/me/contacts/')
          .top(100000)
          // .select('givenName,surname,jobTitle,businessAddress,businessHomePage,businessPhones,companyName,displayName,emailAddresses,generation,givenName,initials,middleName,mobilePhone,title')
          .select('*')
          // .select('givenName,middleName,surname,emailAddresses')
