...
*/
	...
	function OutputvCard(vCard $vCard,$userid)
	{
		$data['userid'] = $userid;
		// print_r($vCard);
		// echo '<br /><br />';
		...
		{
			$data['fname'] = $Name['FirstName'] == "" || $Name['FirstName'] == null || !isset($Name['FirstName']) ? "" : $Name['FirstName'] ; // fname
			$data['lname'] = $Name['LastName'] == "" || $Name['LastName'] == null || !isset($Name['LastName']) ? "" : strtoupper($Name['LastName']); // lname
			$data['mname'] = $Name['AdditionalNames'] == "" || $Name['AdditionalNames'] == null || !isset($Name['AdditionalNames']) ? "" : $Name['AdditionalNames']; // mname
			$data['cnname'] = $Name['Suffixes'] == "" || $Name['Suffixes'] == null || !isset($Name['Suffixes']) ? "" : $Name['Suffixes']; // mname
			$leadname['firstname'] = $Name['FirstName'];
			$leadname['lastname'] = $Name['LastName'];
			$leadname['userid'] = $userid;
		}

		foreach ($vCard -> ORG as $Organization)
		{
			$data['companyname'] = $Organization['Name'] == "" || $Organization['Name'] == null || !isset($Organization['Name']) ? "" : strtoupper($Organization['Name']); // companyname
		}

		$data['website'] = !isset($vCard->url[0]) || $vCard->url[0] == "" || $vCard->url[0] == null ? "" : $vCard->url[0];
		$data['jobtitle'] = $vCard->title[0] == "" || $vCard->title[0] == null || !isset($vCard->title[0]) ? "" : $vCard->title[0];
		$data['dob'] = !isset($vCard->bday[0]) || $vCard->bday[0] == "" || $vCard->bday[0] == null ? "1900-01-01" : formatDate("Y-m-d",$vCard->bday[0]);
		...
		if ($vCard -> TEL)
		{
			...
			{
				switch(strtolower($Tel['Type'][0])){
					case 'work': $data['busphno'] = $Tel['Value'] == "" || $Tel['Value'] == null || !isset($Tel['Value']) ? "" : $Tel['Value']; break;
					case 'home': $data['homphno'] = $Tel['Value'] == "" || $Tel['Value'] == null || !isset($Tel['Value']) ? "" : $Tel['Value']; break;
					case 'cell': $data['mobphno'] = $Tel['Value'] == "" || $Tel['Value'] == null || !isset($Tel['Value']) ? "" : $Tel['Value']; break;
					default: break;
				}
			}
		}
		...
		if ($vCard -> EMAIL)
		{
			...
			{
				$data['emailaddr'] = $Email['Value'] == "" || $Email['Value'] == null || !isset($Email['Value']) ? "" : $Email['Value'];
			}
		}
		// print_r($data);
		// exit();
	    ...
	    $data['cnt'] = ($leads->getTotalLeads($userid) + 1);
	    $exist = $leads->chkLeadExist($leadname);
	    // echo $exist;
	    // exit();
	    if($exist == 0){
	    	$res = $leads->saveVCard($data);
	    }else{
	    	$res = $leads->saveVCardDuplicate($data);
	    	// echo $res;
	    	// exit();
	    }
	}
